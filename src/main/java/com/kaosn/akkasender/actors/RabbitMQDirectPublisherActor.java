...
public class RabbitMQDirectPublisherActor extends AbstractActor {
  ...
  private final RabbitMQPublisherContext context;
  private Channel messageChannel;
  private Connection connection;

  public static Props props(final RabbitMQPublisherContext context) {
    return Props.create(RabbitMQDirectPublisherActor.class, context);
  }

  public RabbitMQDirectPublisherActor(final RabbitMQPublisherContext context) {
    this.context = context;
  }

  @Override
  public void preStart() throws Exception {
    this.connection = ActorUtils.askAndWait(context.getConnectionActor(), CREATE_CONNECTION);
  ...
  private void connectExchangeWithQueue() throws IOException {
    this.messageChannel.exchangeDeclare(context.getExchangeName(),
        EXCHANGE_TYPE,true);
    this.messageChannel.queueDeclare(context.getQueueName(),
  ...
  private void publishMessage(final String message) throws IOException {
    this.messageChannel.basicPublish(
        this.context.getExchangeName(),
        this.context.getRoutingKey(),
        null,
        message.getBytes());
